...
| **etl-suppliers** | Фоновые задачи поиска поставщиков: Yandex Search + Perplexity + краулинг сайтов (headless Chromium). Масштабируется горизонтально — по умолчанию 3 реплики |
| **etl-compare** | Фоновые задачи сравнения ТЗ↔КП. По умолчанию 2 реплики |
...
| `GISP_MAX_CONCURRENT` | `5` | Макс. параллельных Selenium-сессий |
| `ETL_SUPPLIERS_REPLICAS` | `3` | Количество реплик воркера supplier_search |
| `ETL_COMPARE_REPLICAS` | `2` | Количество реплик воркера lot_comparison |
| `LLM_RETRY_MAX_ATTEMPTS` | `3` | Повторы при 408/409/429/5xx (экспоненциальный backoff) |
| `LLM_RETRY_BASE_DELAY_S` | `1.0` | Базовая задержка backoff в секундах (1→2→4) |
...
| `SUPERADMIN_EMAIL` | `qwadro@mail.ru` | Email суперадмина (защищён от изменений другими админами) |
| `UPLOADS_DIR` | `/app/data/uploads` | Директория для хранения оригиналов ТЗ/КП |
...
### Администрирование
- `GET /admin/dashboard` — сводка (пользователи, закупки, pending-заявки)
- `GET /admin/users` / `PATCH /admin/users/{id}/active` — модерация доступа
- `DELETE /admin/users/{id}` — анонимизация аккаунта (152-ФЗ soft-delete)
- `GET /admin/users/{id}/detail` — профиль + LLM usage + файлы
- `GET /admin/queue` — глубина очереди задач + алерты (для мониторинга)
- `GET /admin/usage` — расходы на LLM/Search API по каналам и операциям
...
1. **test / Python syntax** — `py_compile` всего `app/` и `etl/`
2. **test / JavaScript syntax** — `node --check` всего `frontend/js/`
3. **test / HTML smoke** — `index.html` существует и ссылается на `app.js`
4. **deploy / SSH** (только на `push` в `main`, не на PR):
   ...
   - `docker compose up -d --force-recreate --scale etl-suppliers=3 --scale etl-compare=2`
...
PR'ы запускают только `test` — deploy стартует после мержа в `main`.
